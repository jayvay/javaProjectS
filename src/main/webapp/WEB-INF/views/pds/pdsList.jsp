 	...
 	<script>
    	...
    	let pwd = $("#myModal #pwd").val();
 		...
 		function downNumCheck(idx) {
			$.ajax({
				url : "${ctp}/pds/pdsDownNumPlus",
				type : "post",
				data : {idx : idx},
				success : function(res) {
					if(res == "1") $("#downNum").load(location.href+ ' #downNum');
				},
				error : function() {
					alert("전송 오류");
				}
 		...
 		function pageSizeCheck() {
			...
			location.href = "pdsList?part=${part}&pageSize="+pageSize;
		}
	...
	<div class="container">
		...
		<table class="table table-borderless m-0 p-0">
			<tr>
				...
				<td class="text-right">
					<a href="pdsInput?part=${pageVO.part}" class="btn btn-primary">자료올리기</a>
				</td>
			</tr>
		</table>
		<table class="table table-hover text-center" id="downNum">
			...
			<c:forEach var="vo" items="${vos}" varStatus="st">
          	...
          	<c:if test="${vo.hour_diff <= 24}">
					...
					<td>${vo.downNum}</td>
					<td>
						<a href="#" onclick="modalView('${vo.title}','${vo.nickName}','${vo.FDate}','${vo.part}','${vo.FName}','${vo.FSName}','${vo.FSize}','${vo.downNum}')" class="badge badge-info" data-toggle="modal" data-target="#myInfoModal">상세보기</a><br/>
