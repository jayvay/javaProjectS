...
<head>
  ...
  <script>
  	...
  	let nickCheckSw = 0;	//닉네임 중복확인을 했는가
      ...
      let regURL = /^(https?:\/\/)?([a-z\d\.-]+)\.([a-z\.]{2,6})([\/\w\.-]*)*\/?$/;
    	...
    	let submitFlag = 0; //submit 할 것인가
    	
  		if(!regNickName.test(nickName)) {
        alert("닉네임은 한글만 사용할 수 있습니다");
    	...
    	else {		// 전화번호를 입력하지 않을시 DB에 '010- - '의 형태로 저장
    		tel2 = " ";
    		tel3 = " ";
    		tel = tel1 + "-" + tel2 + "-" + tel3;
    		submitFlag = 1;
    	...
    	let extraAddress = myform.extraAddress.value + " ";
  		...
  		// 전송 전에 사진 파일에 관한 사항들을 체크한다.
  		let fName = document.getElementById("file").value;
    	 
    	if(fName.trim() != "") {
	    	let ext = fName.substring(fName.lastIndexOf(".")+1).toLowerCase();	//파일명은 무조건 소문자로
	    	let maxSize = 1024 * 1024 * 5;	//업로드 가능한 최대 용량
	    	let fileSize = document.getElementById("file").files[0].size;	//업로드한 파일의 용량
	    	
	    	if(ext != 'jpg' && ext != 'gif' && ext != 'png' && ext != 'zip' && ext != 'hwp' && ext != 'ppt' && ext != 'pptx' && ext != 'xlsx') {
	    		alert("파일은 'jgp/gif/png/zip/hwp/ppt/pptx/xlsx' 형식만 업로드할 수 있습니다.");
	    	}
	    	else if(fileSize > maxSize) {
	    		alert("업로드 가능한 파일의 최대 용량은 5MByte입니다.");
	    	}
	    	submitFlag = 1;
    	}
  		
    	// 전송 전 모든 체크가 끝나서 submitFlag가 1이 되면 값들을 서버로 전송한다.
    	if(submitFlag == 1) {
    		if(nickCheckSw == 0) {
...
<div class="container">
  <form name="myform" method="post" class="was-validated">
  	<h2>회원정보수정</h2>
  	<br/>
  	<div>아이디 &nbsp; &nbsp; <b>${sMid}</b></div><br/>
  	<div class="form-group">
  		<label for="nickName">닉네임 &nbsp; &nbsp; <input type="button" value="중복확인" id="nickNameBtn" onclick="nickCheck()" class="btn btn-secondary btn-sm"/></label>
  		<input type="text" value="${vo.nickName}" name="nickName" id="nickName" placeholder="닉네임을 입력하세요" required class="form-control"/>
  	</div>
  	<div class="form-group">
  		<label for="nickName">성명 </label>
  		<input type="text" value="${vo.name}" name="name" id="name" placeholder="이름을 입력하세요" required class="form-control"/>
  	</div>
  	<div class="form-group">
  		<label for="email1">이메일 </label>
  		<div class="input-group mb-3">
  			<c:set var="email" value="${fn:split(vo.email,'@')}" />
	  		<input type="text" value="${email[0]}" name="email1" id="email1" placeholder="이메일을 입력하세요" required class="form-control"/>
  	...
  	<div class="form-group">
  		<div class="form-check-inline"> 성별 &nbsp; &nbsp;
  			<label class="form-check-label">
  				<input type="radio" name="gender" class="form-check-input" value="남자" <c:if test="${vo.gender == '남자'}">checked</c:if>/>남자
  			</label>
  		</div>
  		<div class="form-check-inline">
  			<label class="form-check-label">
  				<input type="radio" name="gender" class="form-check-input" value="여자" <c:if test="${vo.gender == '여자'}">checked</c:if>/>여자
  			</label>
  		</div>
  	</div>
  	<div class="form-group">
      ...
      <input type="text" name="homePage" id="homePage" value="${vo.homePage}" placeholder="홈페이지를 입력하세요." class="form-control" />
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      <textarea rows="5" name="content" id="content" placeholder="자기소개를 입력하세요." class="form-control">${vo.content}</textarea>
    </div>
    <div class="form-group">
      <div class="form-check-inline">정보공개  &nbsp; &nbsp;
        <label class="form-check-label">
          <input type="radio" name="userInfor" class="form-check-input" value="공개" <c:if test="${vo.userInfor == '공개'}">checked</c:if>/>공개
        </label>
      </div>
      <div class="form-check-inline">
        <label class="form-check-label">
          <input type="radio" name="userInfor" class="form-check-input" value="비공개" <c:if test="${vo.userInfor == '비공개'}">checked</c:if>/>비공개
        </label>
      </div>
    </div>
    <div class="form-group">회원 사진(파일용량:2MByte이내) 
